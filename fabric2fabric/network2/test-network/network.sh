...
function printHelp() {
  ...
  echo "    -ccep <policy>  - Optional, chaincode endorsement policy, using signature policy syntax. The default policy requires an endorsement from Org13 and Org23"
...
function clearContainers() {
  CONTAINER_IDS=$(docker ps -a | awk '($2 ~ /dev-peer0.org13*/) {print $1}')
  ...
  CONTAINER_IDS=$(docker ps -a | awk '($2 ~ /dev-peer0.org23*/) {print $1}')
  if [ -z "$CONTAINER_IDS" -o "$CONTAINER_IDS" == " " ]; then
    echo "---- No containers available for deletion ----"
  else
    docker rm -f $CONTAINER_IDS
...
function removeUnwantedImages() {
  DOCKER_IMAGE_IDS=$(docker images | awk '($1 ~ /dev-peer0.org13*/) {print $3}')
  ...
  DOCKER_IMAGE_IDS=$(docker images | awk '($1 ~ /dev-peer0.org23*/) {print $3}')
  if [ -z "$DOCKER_IMAGE_IDS" -o "$DOCKER_IMAGE_IDS" == " " ]; then
    echo "---- No images available for deletion ----"
  else
    docker rmi -f $DOCKER_IMAGE_IDS
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo "############ Create Org13 Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-org13.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    echo "##########################################################"
    echo "############ Create Org23 Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-org23.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo $'\e[1;32m'"Failed to generate certificates..."$'\e[0m'
  ...
  echo
  echo "Generate CCP files for Org13 and Org23"
...
## call the script to join create the channel and join the peers of org13 and org23
...
function networkDown() {
  # stop org3 containers also in addition to org13 and org23, in case we were running sample to add org3
...
#  docker-compose -f $COMPOSE_FILE_COUCH_ORG3 -f $COMPOSE_FILE_ORG3 down --volumes --remove-orphans
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/org13/msp organizations/fabric-ca/org13/tls-cert.pem organizations/fabric-ca/org13/ca-cert.pem organizations/fabric-ca/org13/IssuerPublicKey organizations/fabric-ca/org13/IssuerRevocationPublicKey organizations/fabric-ca/org13/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/org23/msp organizations/fabric-ca/org23/tls-cert.pem organizations/fabric-ca/org23/ca-cert.pem organizations/fabric-ca/org23/IssuerPublicKey organizations/fabric-ca/org23/IssuerRevocationPublicKey organizations/fabric-ca/org23/fabric-ca-server.db'
