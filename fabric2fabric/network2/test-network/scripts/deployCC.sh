...
if [ "$CC_SRC_PATH" = "NA" ]; then
	...
	elif [ "$CC_NAME" = "resourceregistry" ]; then
		echo $'\e[0;32m'resourceregistry$'\e[0m' chaincode
		CC_SRC_PATH="../metachan/resourceregistry"
	elif [ "$CC_NAME" = "crossbranch" ]; then
		echo $'\e[0;32m'crossbranch$'\e[0m' chaincode
		CC_SRC_PATH="../mychannel/crossbranch"
	elif [ "$CC_NAME" = "verifyregistry" ]; then
		echo $'\e[0;32m'verifyregistry$'\e[0m' chaincode
		CC_SRC_PATH="../mychannel/verifyregistry"
	elif [ "$CC_NAME" = "verifynetwork1" ]; then
		echo $'\e[0;32m'verifyregistry$'\e[0m' chaincode
		CC_SRC_PATH="../mychannel/verifynetwork1"
...
approveForMyOrg() {
	...
	setGlobals $ORG
	set -x
	peer lifecycle chaincode approveformyorg -o localhost:7250 --ordererTLSHostnameOverride orderer2.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
	res=$?
	set +x
	cat log.txt
...
commitChaincodeDefinition() {
	...
	set -x
	peer lifecycle chaincode commit -o localhost:7250 --ordererTLSHostnameOverride orderer2.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} $PEER_CONN_PARMS --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
	res=$?
	set +x
	cat log.txt
...
chaincodeInvokeInit() {
	...
	peer chaincode invoke -o localhost:7250 --ordererTLSHostnameOverride orderer2.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS --isInit -c ${fcn_call} >&log.txt
	res=$?
	set +x
	cat log.txt
...
## Install chaincode on peer0.org13 and peer0.org23
echo "Installing chaincode on peer0.org13..."
installChaincode 1
echo "Install chaincode on peer0.org23..."
installChaincode 2

## query whether the chaincode is installed
queryInstalled 1

## approve the definition for org13
approveForMyOrg 1

## check whether the chaincode definition is ready to be committed
## expect org13 to have approved and org23 not to
checkCommitReadiness 1 "\"Org13MSP\": true" "\"Org23MSP\": false"
checkCommitReadiness 2 "\"Org13MSP\": true" "\"Org23MSP\": false"

## now approve also for org23
approveForMyOrg 2

## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness 1 "\"Org13MSP\": true" "\"Org23MSP\": true"
checkCommitReadiness 2 "\"Org13MSP\": true" "\"Org23MSP\": true"
