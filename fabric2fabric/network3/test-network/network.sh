...
function printHelp() {
  ...
  echo "    -ccep <policy>  - Optional, chaincode endorsement policy, using signature policy syntax. The default policy requires an endorsement from Org12 and Org22"
...
function clearContainers() {
  CONTAINER_IDS=$(docker ps -a | awk '($2 ~ /dev-peer0.org12*/) {print $1}')
  ...
  CONTAINER_IDS=$(docker ps -a | awk '($2 ~ /dev-peer0.org22*/) {print $1}')
  if [ -z "$CONTAINER_IDS" -o "$CONTAINER_IDS" == " " ]; then
    echo "---- No containers available for deletion ----"
  else
    docker rm -f $CONTAINER_IDS
...
function removeUnwantedImages() {
  DOCKER_IMAGE_IDS=$(docker images | awk '($1 ~ /dev-peer0.org12*/) {print $3}')
  ...
  DOCKER_IMAGE_IDS=$(docker images | awk '($1 ~ /dev-peer0.org22*/) {print $3}')
  if [ -z "$DOCKER_IMAGE_IDS" -o "$DOCKER_IMAGE_IDS" == " " ]; then
    echo "---- No images available for deletion ----"
  else
    docker rmi -f $DOCKER_IMAGE_IDS
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo "############ Create Org12 Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-org12.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    echo "##########################################################"
    echo "############ Create Org22 Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-org22.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo $'\e[1;32m'"Failed to generate certificates..."$'\e[0m'
  ...
  echo
  echo "Generate CCP files for Org12 and Org22"
...
## call the script to join create the channel and join the peers of org12 and org22
...
function networkDown() {
  # stop org3 containers also in addition to org12 and org22, in case we were running sample to add org3
...
#  docker-compose -f $COMPOSE_FILE_COUCH_ORG3 -f $COMPOSE_FILE_ORG3 down --volumes --remove-orphans
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/org12/msp organizations/fabric-ca/org12/tls-cert.pem organizations/fabric-ca/org12/ca-cert.pem organizations/fabric-ca/org12/IssuerPublicKey organizations/fabric-ca/org12/IssuerRevocationPublicKey organizations/fabric-ca/org12/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/org22/msp organizations/fabric-ca/org22/tls-cert.pem organizations/fabric-ca/org22/ca-cert.pem organizations/fabric-ca/org22/IssuerPublicKey organizations/fabric-ca/org22/IssuerRevocationPublicKey organizations/fabric-ca/org22/fabric-ca-server.db'
