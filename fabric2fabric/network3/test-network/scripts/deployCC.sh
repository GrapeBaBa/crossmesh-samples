...
approveForMyOrg() {
	...
	setGlobals $ORG
	set -x
	peer lifecycle chaincode approveformyorg -o localhost:7150 --ordererTLSHostnameOverride orderer1.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
	res=$?
	set +x
	cat log.txt
...
commitChaincodeDefinition() {
	...
	set -x
	peer lifecycle chaincode commit -o localhost:7150 --ordererTLSHostnameOverride orderer1.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} $PEER_CONN_PARMS --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
	res=$?
	set +x
	cat log.txt
...
chaincodeInvokeInit() {
	...
	peer chaincode invoke -o localhost:7150 --ordererTLSHostnameOverride orderer1.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS --isInit -c ${fcn_call} >&log.txt
	res=$?
	set +x
	cat log.txt
...
## Install chaincode on peer0.org12 and peer0.org22
echo "Installing chaincode on peer0.org12..."
installChaincode 1
...
## approve the definition for org12
approveForMyOrg 1

## check whether the chaincode definition is ready to be committed
## expect org12 to have approved and org22 not to
checkCommitReadiness 1 "\"Org12MSP\": true" "\"Org22MSP\": false"
checkCommitReadiness 2 "\"Org12MSP\": true" "\"Org22MSP\": false"

## now approve also for org22
approveForMyOrg 2

## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness 1 "\"Org12MSP\": true" "\"Org22MSP\": true"
checkCommitReadiness 2 "\"Org12MSP\": true" "\"Org22MSP\": true"
